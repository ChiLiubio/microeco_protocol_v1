...
p1 <- res_table %>%
	ggboxplot(x = "method", y = "power", color = "method", palette = "aaas", add = "mean", xlab = "", ylab = "Power", size = 0.6, width = 0.618)
p1 <- ggpar(p1, legend = "none", x.text.angle = 30) + tmp_theme

p2 <- res_table %>%
	ggboxplot(x = "method", y = "fdr", color = "method", palette = "aaas", add = "mean", xlab = "", ylab = "FDR", size = 0.6, width = 0.618)
